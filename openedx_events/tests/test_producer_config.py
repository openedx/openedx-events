...
class ProducerConfiguratonTest(TestCase):
    ...
    @patch("openedx_events.apps.get_producer")
    def test_send_events_with_custom_metadata_not_replayed_by_handler(
        self, mock_producer, mock_logger
    ):
        """
        ...
        XBLOCK_PUBLISHED.send_event_with_custom_metadata(
            metadata, xblock_info=self.xblock_info
        )
    ...
    def test_configuration_is_validated(self):
        ...
        with override_settings(EVENT_BUS_PRODUCER_CONFIG=[]):
            with pytest.raises(
                ProducerConfigurationError, match="should be a dictionary"
            ):
                apps.get_app_config("openedx_events").ready()

        with override_settings(EVENT_BUS_PRODUCER_CONFIG={"invalid.event.type": {}}):
            with pytest.raises(
                ProducerConfigurationError, match="No OpenEdxPublicSignal of type"
            ):
                apps.get_app_config("openedx_events").ready()
        ...
        with override_settings(
            EVENT_BUS_PRODUCER_CONFIG={
                "org.openedx.content_authoring.xblock.deleted.v1": {"topic": ""}
            }
        ):
            with pytest.raises(
                ProducerConfigurationError,
                match="One of the configuration objects is not a dictionary",
            ):
                apps.get_app_config("openedx_events").ready()
        ...
        with override_settings(
            ...
            with pytest.raises(
                ProducerConfigurationError, match="missing 'event_key_field' key."
            ):
                apps.get_app_config("openedx_events").ready()
