...
class OpenEdxPublicSignalTestCache(FreezeSignalCacheMixin, TestCase):
    ...
    def test_generate_signal_metadata(
        self, datetime_mock, socket_mock, events_package_mock
    ):
        """
    ...
    @patch("openedx_events.data.openedx_events")
    @patch("openedx_events.data.socket")
    def test_generate_signal_metadata_with_valid_time(
        self, socket_mock, events_package_mock
    ):
        """
    ...
    @patch(
        "openedx_events.tooling.format_responses",
        autospec=True,
        return_value="fake-output",
    )
    def test_send_robust_event_successfully(
        self, format_responses_mock, log_mock, fake_metadata
    ):
        """
        ...
        with receivers_attached(
            self.public_signal, [self.ok_receiver, self.error_receiver]
        ):
            self.public_signal.send_event(user=self.user_mock)

        self.ok_receiver.assert_called_once_with(
            signal=self.public_signal,
            sender=None,
            user=self.user_mock,
            metadata=expected_metadata,
            from_event_bus=False,
        ...
        format_responses_mock.assert_called_once_with(
            [(self.ok_receiver, "success"), (self.error_receiver, self.receiver_error)],
            depth=2,
        )
    ...
    def test_send_event_with_custom_metadata(self, mock_send_event_with_metadata):
        ...
        expected_response: Any = "mock-response"
...
class TestLoadAllSignals(FreezeSignalCacheMixin, TestCase):
    ...
    def setUp(self):
        ...
        super().setUp()
        self.old_signal_modules: dict[str, Any] = {}
