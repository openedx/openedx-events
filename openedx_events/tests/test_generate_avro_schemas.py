...
class TestGenerateAvroCommand(FreezeSignalCacheMixin, TestCase):
    ...
    def test_generate_multiple_specified_schemas(self):
        ...
        with patch("builtins.open", mock_open()) as mock_file:
            ...
            mock_file.assert_has_calls(
                [
                    call(
                        f"{TestGenerateAvroCommand.folder_path}/simple+signal_schema.avsc",
                        "w",
                    ),
                    call(
                        f"{TestGenerateAvroCommand.folder_path}/simple+signal+B_schema.avsc",
                        "w",
                    ),
                ],
    ...
    def test_generate_all(self):
        load_all_signals()
        expected_files = [
            f"{TestGenerateAvroCommand.folder_path}/{signal.event_type.replace('.', '+')}_schema.avsc"
            for signal in OpenEdxPublicSignal.all_events()
            if signal.event_type not in KNOWN_UNSERIALIZABLE_SIGNALS
        ]
        with patch("builtins.open", mock_open()) as mock_file:
            call_command(Command(), all=True)
            mock_file.assert_has_calls(
                [call(file, "w") for file in expected_files], any_order=True
            )
    ...
    def test_create_schema_dir_if_not_exists(self, mock_makedirs):
        ...
        with patch(
            ...
            lambda path: (
                False
                if path == TestGenerateAvroCommand.folder_path
                else old_exists(path)
            ),
        ):
