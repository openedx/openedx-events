...
class TestLoader(TestCase):
    ...
    def test_unconfigured(self):
        with assert_warnings(
            ["Event Bus setting DOES_NOT_EXIST is missing; component will be inactive"]
        ):
            loaded = _try_load(
...
class TestConsumer(TestCase):
    ...
    def test_default_does_nothing(self):
        ...
        with assert_warnings(
            [
                "Event Bus setting EVENT_BUS_CONSUMER is missing; component will be inactive"
            ]
        ):
            consumer = make_single_consumer(
                topic="test", group_id="test", signal=SESSION_LOGIN_COMPLETED
            )

        with pytest.raises(
            Exception, match=re.escape("Cannot consume events; no consumer configured.")
        ):
