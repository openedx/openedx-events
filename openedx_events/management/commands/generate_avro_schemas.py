...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser: Any) -> None:
        ...
        parser.add_argument(
            "--all", action="store_true", help="Write schema for all event types"
        )
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        if options["all"]:
            ...
            signals = [
                ...
                for event_type in options["types"]
            ]

        for signal in signals:
            if signal.event_type in KNOWN_UNSERIALIZABLE_SIGNALS:
                logger.info(
                    f"Known unserializable signal: {signal.event_type}. Skipping."
                )
                continue
            ...
            if os.path.exists(full_file_name):
                confirmation = input(
                    f"Warning: overwriting schema for {signal.event_type}. It is recommended to leave"
